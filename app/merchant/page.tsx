...
export default async function MerchantRolePage() {
  ...
  let merchants = (primaryMerchants.data ?? []) as MerchantRow[];
  if (!merchants.length && primaryMerchants.error && isMissingLogoColumn(primaryMerchants.error.message)) {
    ...
    merchants = ((legacyMerchants.data ?? []) as Array<Omit<MerchantRow, "logo_url">>).map(
      (merchant) => ({ ...merchant, logo_url: null }),
    );
  }
  ...
  return (
    <main className="mx-auto w-full max-w-2xl space-y-5 px-4 py-6">
      <section className="rounded-3xl border border-[#262626] bg-[#1A1A1A] p-6">
        ...
        <h1 className="mt-3 text-2xl font-semibold">Choose a business</h1>
        <p className="mt-2 text-sm text-[#A1A1AA]">
          Use your merchant role and continue with one of your business profiles.
        </p>
