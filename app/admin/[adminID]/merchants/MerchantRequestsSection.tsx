...
type MerchantRequestsSectionProps = {
  requests: MerchantRequest[];
};

export default function MerchantRequestsSection({
  requests,
}: MerchantRequestsSectionProps) {
  const [items, setItems] = useState<MerchantRequest[]>(requests);
  ...
  return (
    <section className="rounded-3xl border border-[#1F2937] bg-[#0F172A] p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">Merchant requests</h2>
          <p className="mt-2 text-sm text-[#94A3B8]">
            Validate new merchant applications and approve storefront access.
          </p>
        </div>
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <p className="text-sm text-[#94A3B8]">
                      Contact: {request.contactName} · {request.phone}
                    </p>
                  </div>
                  <div className="text-sm text-[#94A3B8]">
                    <p>Business email: {request.email}</p>
                    {request.address ? (
                      <p>Address: {request.address}</p>
                    ) : null}
                  </div>
                  <div className="text-xs text-[#64748B]">
                    Submitted {formatter.format(new Date(request.createdAt))}
                  </div>
                  {request.user ? (
                    <div className="rounded-2xl border border-[#1F2937] bg-[#0F172A] px-4 py-3 text-sm text-[#94A3B8]">
                      ...
                      <p className="text-xs text-[#64748B]">
                        Joined {formatter.format(new Date(request.user.createdAt))}
                      </p>
